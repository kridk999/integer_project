...
@variable(mo, 0 <= y[1:15] <= 1)
@variable(mo, 0 <= x[1:15,1:15] <= 1)


n = 15 #Defining the number of variables
...
@constraint(mo, [j = 1:n], sum(a[i]*x[i,j]  for i = 1:n if i!=j) <= (v[j]-a[j])*y[j])
#∑aᵢ*xᵢⱼ for j = 1:n, i ≠ j + yᵢ  = 1, ∀i
@constraint(mo, [i = 1:n], sum(x[i,j] for j = 1:n if i!=j) + y[i]  == 1)

# Constraint to ensure liquid is not moved from a container to itself
@constraint(mo, [i=1:n], x[i,i] == 0)

@constraint(mo, x[11, 6] == 1)
